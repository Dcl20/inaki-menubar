...
process.on('uncaughtException', (err) => {
  console.error('[inaki-quick] Uncaught exception:', err.message);
});
process.on('unhandledRejection', (reason) => {
  console.error('[inaki-quick] Unhandled rejection:', reason);
});
...
app.setAsDefaultProtocolClient('inaki-quick');
...
app.whenReady().then(() => {
  ...
  const protocolArg = process.argv.find(a => a.startsWith('inaki-quick://'));
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip(`Quick by Iñaki (${shortcut})`);
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .link:hover { background: #b91c1c; }
  .shortcut { font-size: 11px; color: #555; margin-top: 24px; }
</style>
</head>
<body>
  <div class="logo"><span>quick</span></div>
  <div class="subtitle">by Iñaki</div>
  <div class="instructions">
    <div class="step">Para conectar tu cuenta:</div>
    <div class="step">1. Abre <strong>iñaki.app</strong> en tu navegador</div>
    <div class="step">2. Presiona el <strong>rayito ⚡</strong> abajo a la derecha</div>
    <div class="step">3. Haz clic en <strong>"Conectar mi cuenta"</strong></div>
...
function injectUI() {
  ...
  win.webContents.executeJavaScript(`
    (function() {
      ...
      if (!document.getElementById('inaki-bar')) {
      ...
      // --- Escape to hide ---
      if (!window._inakiKeys) {
        window._inakiKeys = true;
        document.addEventListener('keydown', function(e) {
          if (e.key === 'Escape' && window.inaki) window.inaki.hide();
        });
      }

      // --- Drag & drop images ---
      if (!window._inakiDrop) {
        window._inakiDrop = true;
        ...
        document.addEventListener('dragenter', function(e) {
          e.preventDefault();
          if (!document.getElementById('inaki-drop-overlay')) {
            var overlay = document.createElement('div');
            overlay.id = 'inaki-drop-overlay';
            overlay.style.cssText = 'position:fixed;inset:0;z-index:99998;background:rgba(220,38,38,0.1);border:2px dashed rgba(220,38,38,0.5);border-radius:16px;display:flex;align-items:center;justify-content:center;pointer-events:none';
            overlay.innerHTML = '<div style="background:rgba(0,0,0,0.7);padding:8px 16px;border-radius:10px;color:#DC2626;font-size:13px;font-weight:600">Suelta la imagen aqui</div>';
        ...
        document.addEventListener('dragleave', function(e) {
          if (e.relatedTarget === null || e.relatedTarget === document.documentElement) {
            var ov = document.getElementById('inaki-drop-overlay');
            if (ov) ov.remove();
          }
        });
...
async function toggleWindow() {
  ...
  // 2. Now show the Inaki window at saved position (or default)
  const saved = loadBounds();
  const defaults = getDefaultPosition();
  const bounds = saved || { x: defaults.x, y: defaults.y, width: 340, height: 280 };
...
app.on('second-instance', (_event, argv) => {
  ...
  const protocolArg = argv.find(a => a.startsWith('inaki-quick://'));
  if (protocolArg) {
...
function handleProtocolUrl(url) {
  try {
    if (typeof url !== 'string' || !url.startsWith('inaki-quick://')) {
      console.warn('[inaki-quick] Invalid protocol URL format');
      return;
    }

    const parsed = new URL(url);
    if (parsed.protocol !== 'inaki-quick:') return;

    const refreshToken = parsed.searchParams.get('rt');
    if (!refreshToken || refreshToken.length < 10 || refreshToken.length > 4096) {
      console.warn('[inaki-quick] Invalid or missing refresh token');
      return;
    }
    ...
    if (!sanitized || sanitized.length < 10) {
      console.warn('[inaki-quick] Token failed sanitization');
      return;
    }
    ...
    console.error('[inaki-quick] Protocol handler error:', e.message);
  }
}
